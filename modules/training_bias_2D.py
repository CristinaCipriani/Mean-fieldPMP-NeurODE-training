...
def F1(nt,x1,x2,theta_in):
    ...
    th13 = theta_in[nt,0,2]
    return np.tanh(th11*x1 + th12*x2 + th13) 
    
def F2(nt,x1,x2,theta_in):
    th21 = theta_in[nt,1,0]
    th22 = theta_in[nt,1,1]
    th23 = theta_in[nt,1,2]
    return np.tanh(th21*x1 + th22*x2 + th23)

def der_omega_F1(nt, x1, x2, theta_in): 
...
def der_omega_F2(nt, x1, x2, theta_in): 
...
def der_sigma_F1(nt, x1, x2, theta_in): 
    mat = np.zeros((2,1))
    mat[0] = 1-F1(nt,x1, x2,theta_in)**2
    return mat
       
def der_sigma_F2(nt, x1, x2, theta_in):
    mat = np.zeros((2,1))
    mat[1] = 1-F2(nt,x1, x2,theta_in)**2
    return mat
        ...
        if diff == "fu_old":
            ...
            u[0,0,0:Ny1+2,0:Ny2+2] = (u_n[0,0,0:Ny1+2,0:Ny2+2] 
                          - (F1(Nt-2-n,x1_b[0],x2_b[0],theta)*(-dt))/dx * 
                            (2*u_n[1,0,0:Ny1+2,0:Ny2+2]-(3/2)*u_n[0,0,0:Ny1+2,0:Ny2+2]-(1/2)*u_n[2,0,0:Ny1+2,0:Ny2+2])*
                          - (F2(n,x1_b[0],x2_b[0],theta)*(-dt))/dx * 
                            (2*u_n[0,1,0:Ny1+2,0:Ny2+2]-(3/2)*u_n[0,0,0:Ny1+2,0:Ny2+2]-(1/2)*u_n[0,2,0:Ny1+2,0:Ny2+2])
                         )
            u[0,Nx2+1,0:Ny1+2,0:Ny2+2] = (u_n[0,Nx+1,0:Ny1+2,0:Ny2+2] 
                              - (F1(n,x1_b[0],x2_b[Nx2+1],theta)*(-dt))/dx 
                              * (2*u_n[1,Nx2+1,0:Ny1+2,0:Ny2+2]-(3/2)*u_n[0,Nx2+1,0:Ny1+2,0:Ny2+2]-(1/2)*u_n[2,Nx2+1,0:Ny1+2,0:Ny2+2])
                              - (F2(n,x1_b[0],x2_b[Nx2+1],theta)*(-dt))/dx 
                              * ((3/2)*u_n[0,Nx2+1,0:Ny1+2,0:Ny2+2]-2*u_n[0,Nx2,0:Ny1+2,0:Ny2+2]+(1/2)*u_n[0,Nx2-1,0:Ny1+2,0:Ny2+2])
                              )
            u[Nx1+1,0,0:Ny1+2,0:Ny2+2] = (u_n[Nx1+1,0,0:Ny1+2,0:Ny2+2] 
                              - (F1(n,x1_b[Nx1+1],x2_b[0],theta)*(-dt))/dx 
                              * ((3/2)*u_n[Nx1+1,0,0:Ny1+2,0:Ny2+2]-2*u_n[Nx1,0,0:Ny1+2,0:Ny2+2]+(1/2)*u_n[Nx1-1,0,0:Ny1+2,0:Ny2+2])
                              - (F2(n,x1_b[Nx1+1],x2_b[0],theta)*(-dt))/dx 
                              * (2*u_n[Nx1+1,1,0:Ny1+2,0:Ny2+2]-(3/2)*u_n[Nx1+1,0,0:Ny1+2,0:Ny2+2]-(1/2)*u_n[Nx1+1,2,0:Ny1+2,0:Ny2+2])
                              )
            u[Nx1+1, Nx2+1,0:Ny1+2,0:Ny2+2] = (u_n[Nx1+1,Nx2+1,0:Ny1+2,0:Ny2+2] 
                              - (F1(n,x1_b[Nx1+1],x2_b[Nx2+1],theta)*(-dt))/dx 
                              * ((3/2)*u_n[Nx1+1,Nx2+1,0:Ny1+2,0:Ny2+2]-2*u_n[Nx1,Nx2+1,0:Ny1+2,0:Ny2+2]+(1/2)*u_n[Nx1-1,Nx2+1,0:Ny1+2,0:Ny2+2])
                              - (F2(n,x1_b[Nx1+1],x2_b[Nx2+1],theta)*(-dt))/dx 
                              * ((3/2)*u_n[Nx1+1,Nx2+1,0:Ny1+2,0:Ny2+2]-2*u_n[Nx1+1,Nx2,0:Ny1+2,0:Ny2+2]+(1/2)*u_n[Nx1+1,Nx2-1,0:Ny1+2,0:Ny2+2])
                      )
            #Boundary lines
            for l in range(0,Ny1+2):
                    for m in range(0,Ny2+2):
                        u[0,1:Nx2+1,l,m] = (u_n[0,1:Nx2+1,l,m] 
                                            - (F1(n,x1_b[0],x2_b[1:Nx2+1],theta)*(-dt))/dx 
                                            * (2*u_n[1,1:Nx2+1,l,m]-(3/2)*u_n[0,1:Nx2+1,l,m]-(1/2)*u_n[2,1:Nx2+1,l,m])
                                            - (F2(n,x1_b[0],x2_b[1:Nx2+1],theta)*(-dt))/(2*dx)
                                            *  (u_n[0,2:Nx2+2,l,m]-u_n[0,0:Nx2,l,m])
                                            )
                        u[1:Nx1+1,0,l,m] = (u_n[1:Nx1+1,0,l,m] 
                                            - (F1(n,x1_b[1:Nx1+1],x2_b[0],theta)*(-dt))/(2*dx) 
                                            * (u_n[2:Nx1+2,0,l,m]-u_n[0:Nx1,0,l,m])
                                            - (F2(n,x1_b[1:Nx1+1],x2_b[0],theta)*(-dt))/dx 
                                            * (2*u_n[1:Nx1+1,1,l,m]-(3/2)*u_n[1:Nx1+1,0,l,m]-(1/2)*u_n[1:Nx1+1,2,l,m])
                                            )
                        u[1:Nx1+1,Nx2+1,l,m] = (u_n[1:Nx1+1,Nx2+1,l,m] 
                                                - (F1(n,x1_b[1:Nx1+1],x2_b[Nx2+1],theta)*(-dt))/(2*dx) 
                                                * (u_n[2:Nx1+2,Nx2+1,l,m]-u_n[0:Nx1,Nx2+1,l,m])
                                                - (F2(n,x1_b[1:Nx1+1],x2_b[Nx2+1],theta)*(-dt))/dx 
                                                * ((3/2)*u_n[1:Nx1+1,Nx2+1,l,m]-2*u_n[1:Nx1+1,Nx2,l,m]+(1/2)*u_n[1:Nx1+1,Nx2-1,l,m])
                                                )
                        u[Nx+1, 1:Nx2+1,l,m] = (u_n[Nx1+1,1:Nx2+1,l,m]
                                                - (F1(n,x1_b[Nx1+1],x2_b[1:Nx2+1],theta)*(-dt))/dx 
                                                * ((3/2)*u_n[Nx1+1,1:Nx2+1,l,m]-2*u_n[Nx1,1:Nx2+1,l,m]+(1/2)*u_n[Nx1-1,1:Nx2+1,l,m])
                                                - (F2(n,x1_b[Nx1+1],x2_b[1:Nx2+1],theta)*(-dt))/(2*dx) 
                                                * (u_n[Nx1+1,2:Nx2+2,l,m]-u_n[Nx1+1,0:Nx2,l,m])
                                                )
            u[1:Nx1+1,1:Nx2+1,1:Ny1+1,1:Ny2+1] = (u_n[1:Nx1+1,1:Nx2+1,1:Ny1+1,1:Ny2+1] 
                                  - (F1(n,x1_b[1:Nx1+1],x2_b[1:Nx2+1],theta)*(-dt))/(dx1) *
                                      (u_n[1:Nx1+1,1:Nx2+1,1:Ny1+1,1:Ny2+1]-u_n[0:Nx1,1:Nx2+1,1:Ny1+1,1:Ny2+1])*0.5*(1+np.sign(x1_b[1:Nx1+1]))
                                  - (F1(n,x1_b[1:Nx1+1],x2_b[1:Nx2+1],theta)*(-dt))/(dx1) *
                                      (u_n[2:Nx1+2,1:Nx2+1,1:Ny1+1,1:Ny2+1]-u_n[1:Nx1+1,1:Nx2+1,1:Ny1+1,1:Ny2+1])*0.5*(1-np.sign(x1_b[1:Nx1+1]))
                                  - (F2(n,x1_b[1:Nx1+1],x2_b[1:Nx2+1],theta)*(-dt))/(2*dx2) *
                                      (u_n[1:Nx1+1,1:Nx2+1,1:Ny1+1,1:Ny2+1]-u_n[1:Nx1+1,0:Nx2,1:Ny1+1,1:Ny2+1])*0.5*(1+np.sign(x2_b[1:Nx2+1]))
                                  - (F2(n,x1_b[1:Nx1+1],x2_b[1:Nx2+1],theta)*(-dt))/(2*dx2) *
                                      (u_n[1:Nx1+1,2:Nx2+2,1:Ny1+1,1:Ny2+1]-u_n[1:Nx1+1,1:Nx2+1,1:Ny1+1,1:Ny2+1])*0.5*(1-np.sign(x2_b[1:Nx2+1]))
                                 )
...
def parameter_update(Z_trace, g, psi_pos, psi_neg, der_omega_F1, der_omega_F2, der_sigma_F1, der_sigma_F2, theta, Lambda, x1 , x2, dx , dt, Nx, Nt, mid_point, dynamics_plots): 
    d = theta.shape[1]
    ...
    for n in range(0, Nt-1):
        ...
        sol0 = optimize.brentq(root_function_00, -500, 500, args=(Z_trace, g, spl_neg, spl_pos, der_omega_F1, der_omega_F2, Lambda, theta, n))
        theta_new[n,0,0] = sol0
        
        sol1 = optimize.brentq(root_function_01, -500, 500, args=(Z_trace, g, spl_neg, spl_pos, der_omega_F1, der_omega_F2, Lambda, theta, n))
        theta_new[n,0,1] = sol1
        
        sol2 = optimize.brentq(root_function_10, -500, 500, args=(Z_trace, g, spl_neg, spl_pos, der_omega_F1, der_omega_F2, Lambda, theta, n))
        theta_new[n,1,0] = sol2
        
        sol3 = optimize.brentq(root_function_11, -500, 500, args=(Z_trace, g, spl_neg, spl_pos, der_omega_F1, der_omega_F2, Lambda, theta, n))
        theta_new[n,1,1] = sol3
        
        sol4 = optimize.brentq(root_function_02, -50, 50, args=(Z_trace, g, spl_neg, spl_pos, der_sigma_F1, der_sigma_F2, Lambda, theta, n))
        theta_new[n,0,2] = sol4
        ...
        theta_domain = np.linspace(-10,10, 101)
        f0_domain = np.zeros(101)
        f1_domain = np.zeros(101)
        f2_domain = np.zeros(101)
        f3_domain = np.zeros(101)
        ...
        for tt in range(len(theta_domain)):
            # first component
            theta_ext0[n,0,0] = theta_domain[tt] 
            f0_domain[tt] = root_function(Z_trace, g, spl_neg, spl_pos, der_theta_F1, der_theta_F2, der_2_theta_F1, der_2_theta_F2, Lambda, theta, n, printy = False)[0]
            # second component
            theta_ext1[n,0,1] = theta_domain[tt] 
            f1_domain[tt] = root_function(Z_trace, g, spl_neg, spl_pos, der_theta_F1, der_theta_F2, der_2_theta_F1, der_2_theta_F2, Lambda, theta, n, printy = False)[1]
            # third component
            theta_ext2[n,1,0] = theta_domain[tt] 
            f2_domain[tt] = root_function(Z_trace, g, spl_neg, spl_pos, der_theta_F1, der_theta_F2, der_2_theta_F1, der_2_theta_F2, Lambda, theta, n, printy = False)[2]
            # third component
            theta_ext3[n,1,1] = theta_domain[tt] 
            f3_domain[tt] = root_function(Z_trace, g, spl_neg, spl_pos, der_theta_F1, der_theta_F2, der_2_theta_F1, der_2_theta_F2, Lambda, theta, n, printy = False)[3]
            
        # Interpolating all the functions       
        #f0_interp = interpolate.BSpline(theta_domain, f0_domain, k=2)   
        #f1_interp = interpolate.BSpline(theta_domain, f1_domain, k=2)
        #f2_interp = interpolate.BSpline(theta_domain, f2_domain, k=2)
        #f3_interp = interpolate.BSpline(theta_domain, f3_domain, k=2) 
        
        sol0 = optimize.newton(root_function_th, theta[n,:], args=(Z_trace, g, spl_neg, spl_pos, F, Lambda, Nt, d, n), maxiter=500)
        #sol = optimize.bisect(root_function_th, -30, 30, args=(Z_trace, g, spl_neg, spl_pos, F, Lambda, Nt, d, n))
        theta_new[n,:] = sol
        
        #Finding the root of all the functions
        root0 = optimize.root(f0_interp, theta[n,0,0])   
        theta_new[n,0,0] = root0.x
        root1 = optimize.root(f1_interp, theta[n,0,1])   
        theta_new[n,0,1] = root1.x
        root2 = optimize.root(f2_interp, theta[n,1,0])   
        theta_new[n,1,0] = root2.x
        root3 = optimize.root(f3_interp, theta[n,1,1])   
        theta_new[n,1,1] = root3.x

        #if dynamics_plots:
        #    axs[n].scatter(sol, f_interp(sol))    
        #theta_new[n,:] = sol
        ...
        while np.abs(f) > 0.00000001 and itr < 5000:
            if itr == 0 :
                f = root_function(Z_trace, g, spl_neg, spl_pos, F, Lambda, theta, n, False)[0]  #if you want to print put this one to True
                conv = f - 2*Lambda*theta[n,:]
                #print("For n= %s, f_0 has value %s" %(n,conv))  #and this one tooo
            else:
                f = root_function(Z_trace, g, spl_neg, spl_pos, F, Lambda, theta, n, False)[0]
            f_prime = root_function(Z_trace, g, spl_neg, spl_pos, F, Lambda, theta, n, False)[1]
            theta_new[n,:] = theta[n,:] - f_prime*f/np.abs(f_prime)**2
            theta[n,:] = theta_new[n,:]
            if dynamics_plots:
                plt.scatter(theta[n,:], f_interp(theta[n,:]))
            itr += 1
        
        if dynamics_plots:
            axs[n].scatter(theta[n,:], f_interp(theta[n,:]))
            axs[n].set_title("Solution "+ str(np.around(theta[n,:],2)) +" after " + str(itr)+ " iterations" )
        
        
        count = 7000
        #f_value[n] = f_interp(sol)
        f_value[n] = f
        f_0 = 0
        f_prime_0 = 0
...
def root_function(Z_trace, g, spl_neg, spl_pos, der_omega_F1, der_omega_F2, der_sigma_F1, der_sigma_F2, Lambda, theta, n, printy):
    ...
    for i in range(0,N_particles):
        ...
        if Z_trace[n,i,2] == g[n,i,0] and Z_trace[n,i,3] == g[n,i,1] and x_i[0] >= 0 :
            f0 += ( spl_pos(x_i[0], x_i[1], dx=1) * der_omega_F1(n, x_i[0], x_i[1], theta) +
                   spl_pos(x_i[0], x_i[1], dy=1) * der_omega_F2(n, x_i[0], x_i[1], theta)
                   )
            f1 +=  (spl_pos(x_i[0], x_i[1], dx=1) * der_sigma_F1(n, x_i[0], x_i[1], theta) +
                   spl_pos(x_i[0], x_i[1], dy=1) * der_sigma_F2(n, x_i[0], x_i[1], theta)
                   )
            
        if Z_trace[n,i,2] == g[n,i,0] and Z_trace[n,i,3] == g[n,i,1] and x_i[0] < 0 :
            f0 += ( spl_neg(x_i[0], x_i[1], dx=1) * der_omega_F1(n, x_i[0], x_i[1], theta) +
                   spl_neg(x_i[0], x_i[1], dy=1) * der_omega_F2(n, x_i[0], x_i[1], theta)
                   )
            f1 += ( spl_neg(x_i[0], x_i[1], dx=1) * der_sigma_F1(n, x_i[0], x_i[1], theta) +
    ...
    double = False 
    if double == False:
        f0_tot = 2*Lambda[0,0]*theta[n,0,0] + f0[0,0]/N_particles
        f1_tot = 2*Lambda[0,1]*theta[n,0,1] + f0[0,1]/N_particles
        f2_tot = 2*Lambda[1,0]*theta[n,1,0] + f0[1,0]/N_particles
        f3_tot = 2*Lambda[1,1]*theta[n,1,1] + f0[1,1]/N_particles
        f4_tot = 2*Lambda[0,2]*theta[n,0,2] + f1[0]/N_particles
        f5_tot = 2*Lambda[1,2]*theta[n,1,2] + f1[1]/N_particles
    else:
        f0_tot = 2*Lambda[0,0]*theta[n,0,0] + 2 * f0[0,0]/N_particles
        f1_tot = 2*Lambda[0,1]*theta[n,0,1] + 2 * f0[0,1]/N_particles
        f2_tot = 2*Lambda[1,0]*theta[n,1,0] + 2 * f0[1,0]/N_particles
        f3_tot = 2*Lambda[1,1]*theta[n,1,1] + 2 * f0[1,1]/N_particles
...
def root_function_00(theta_00, Z_trace, g, spl_neg, spl_pos, der_omega_F1, der_omega_F2, Lambda, theta, n):
    ...
    for i in range(0,N_particles):
        ...
        #x_i = np.random.multivariate_normal(Z_trace[n,i,:d],0.01*np.identity(d))
        if Z_trace[n,i,2] == g[n,i,0] and Z_trace[n,i,3] == g[n,i,1] and x_i[0] > 0 :
            f += ( spl_pos(x_i[0], x_i[1], dx=1) * der_omega_F1(n, x_i[0], x_i[1], theta) +
                   spl_pos(x_i[0], x_i[1], dy=1) * der_omega_F2(n, x_i[0], x_i[1], theta)
                   )
        if Z_trace[n,i,2] == g[n,i,0] and Z_trace[n,i,3] == g[n,i,1] and x_i[0] <= 0 :
            f += ( spl_neg(x_i[0], x_i[1], dx=1) * der_omega_F1(n, x_i[0], x_i[1], theta) +
                   spl_neg(x_i[0], x_i[1], dy=1) * der_omega_F2(n, x_i[0], x_i[1], theta)
                   )
...
def root_function_01(theta_01, Z_trace, g, spl_neg, spl_pos, der_omega_F1, der_omega_F2, Lambda, theta, n):
...
def root_function_10(theta_10, Z_trace, g, spl_neg, spl_pos, der_omega_F1, der_omega_F2, Lambda, theta, n):
...
def root_function_11(theta_11, Z_trace, g, spl_neg, spl_pos, der_omega_F1, der_omega_F2, Lambda, theta, n):
...
def root_function_02(theta_02, Z_trace, g, spl_neg, spl_pos, der_sigma_F1, der_sigma_F2, Lambda, theta, n):
    theta[n,0,2] = theta_02
    N_particles = Z_trace.shape[1]
    d = theta.shape[1]
    f = np.zeros((d,1))
    ...
    for i in range(0,N_particles):
        ...
        #x_i = np.random.multivariate_normal(Z_trace[n,i,:d],0.01*np.identity(d))
        if Z_trace[n,i,2] == g[n,i,0] and Z_trace[n,i,3] == g[n,i,1] and x_i[0] > 0 :
            f += ( spl_pos(x_i[0], x_i[1], dx=1) * der_sigma_F1(n, x_i[0], x_i[1], theta) +
                   spl_pos(x_i[0], x_i[1], dy=1) * der_sigma_F2(n, x_i[0], x_i[1], theta)
                   )
        if Z_trace[n,i,2] == g[n,i,0] and Z_trace[n,i,3] == g[n,i,1] and x_i[0] <= 0 :
            f += ( spl_neg(x_i[0], x_i[1], dx=1) * der_sigma_F1(n, x_i[0], x_i[1], theta) +
                   spl_neg(x_i[0], x_i[1], dy=1) * der_sigma_F2(n, x_i[0], x_i[1], theta)
                   )
            
    double = False 
    if double == False:
        f4_tot = 2*Lambda[0,2]*theta[n,0,2] + f[0]/N_particles
    else:
        f4_tot = 2*Lambda[0,2]*theta[n,0,2] + 2 * f[0]/N_particles
        
    return f4_tot

def root_function_12(theta_12, Z_trace, g, spl_neg, spl_pos, der_sigma_F1, der_sigma_F2, Lambda, theta, n):
    theta[n,1,2] = theta_12
    N_particles = Z_trace.shape[1]
    d = theta.shape[1]
    f = np.zeros((d,1))
    ...
    double = False 
    if double == False:
        f5_tot = 2*Lambda[1,2]*theta[n,1,2] + f[1]/N_particles
    else:
        f5_tot = 2*Lambda[1,2]*theta[n,1,2] + 2 * f[1]/N_particles
...
def MFOC(N, d, T, dt, R, mu_0, center_left, center_right, y_left, y_right, xmin, xmax, grid_points, theta, F, mid_point, Lambda, num_iterations):
    ...
    thetas = np.zeros((num_iterations+1, Nt-1,d,d+1))
    thetas[0,:] = theta.reshape((Nt-1,d,d+1))
    ...
    for k in range(0,num_iterations):
        ...
        #if dynamics_plots:
        #    fig, axs = plt.subplots(1, Nt-1, figsize=(35,5) )
        #    x_new = np.linspace(xmin, xmax, 200)
        #    for n in range(Nt-1):
        #        axs[Nt-2-n].scatter(x,psi_neg[Nt-1-n,:])
        #        spl = interpolate.BSpline(x,psi_neg[Nt-1-n,:,],k=2)
        #        axs[Nt-2-n].plot(x_new, spl(x_new),'r')
        #        #axs[Nt-2-n].plot(x_new, spl(x_new,nu=1),'g')
        #    fig.tight_layout()
        #    plt.show() 

        #Resolution of the parameter update
        print("Updating the parameter...")
        theta, f_value, count, loss_fct = parameter_update(Z_trace, g, psi_pos, psi_neg, der_omega_F1, der_omega_F2, der_sigma_F1, der_sigma_F2, theta, Lambda, x1 , x2, dx , dt, Nx, Nt, mid_point, dynamics_plots)
        counts[k] = count  
        thetas[k+1,:] = theta.reshape((Nt-1,d,d+1))
                
    plt.figure()
    Z_trace, g = move_forward(dt, Nt, N, Z_all, F, thetas[-1,:].reshape((Nt-1,d,d+1)), mid_point, y_left, y_right)
