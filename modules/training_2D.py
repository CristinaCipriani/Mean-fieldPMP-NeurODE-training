...
def der_2_theta_F1(nt, x1, x2, theta_in):
    th11 = theta_in[nt,0,0]
    th12 = theta_in[nt,0,1] 
    const = -2 * F1(nt,x1,x2,theta_in) * (1-F1(nt,x1, x2,theta_in)**2)
    mat = np.zeros((2,2))
    mat[0,0] = const * x1**2
    mat[0,1] = const * x2**2
    return mat    
    
def der_2_theta_F2(nt, x1, x2, theta_in):
    th21 = theta_in[nt,1,0]
    th22 = theta_in[nt,1,1] 
    const = -2 * F2(nt,x1,x2,theta_in) * (1-F2(nt,x1, x2,theta_in)**2)
    mat = np.zeros((2,2))
    mat[1,0] = const * x1**2
    mat[1,1] = const * x2**2
    return mat
...
def parameter_update(Z_trace, g, psi_pos, psi_neg, der_theta_F1, der_theta_F2, der_2_theta_F1, der_2_theta_F2, theta, Lambda, x1 , x2, dx , dt, Nx, Nt, mid_point, dynamics_plots): 
    d = theta.shape[1]
    ...
    for n in range(0, Nt-1):
        ...
        sol0 = optimize.brentq(root_function_00, -500, 500, args=(Z_trace, g, spl_neg, spl_pos, der_theta_F1, der_theta_F2, der_2_theta_F1, der_2_theta_F2, Lambda, theta, n))
        theta_new[n,0,0] = sol0
        
        sol1 = optimize.brentq(root_function_01, -500, 500, args=(Z_trace, g, spl_neg, spl_pos, der_theta_F1, der_theta_F2, der_2_theta_F1, der_2_theta_F2, Lambda, theta, n))
        theta_new[n,0,1] = sol1
        
        sol2 = optimize.brentq(root_function_10, -500, 500, args=(Z_trace, g, spl_neg, spl_pos, der_theta_F1, der_theta_F2, der_2_theta_F1, der_2_theta_F2, Lambda, theta, n))
        theta_new[n,1,0] = sol2
        
        sol3 = optimize.brentq(root_function_11, -500, 500, args=(Z_trace, g, spl_neg, spl_pos, der_theta_F1, der_theta_F2, der_2_theta_F1, der_2_theta_F2, Lambda, theta, n))
        theta_new[n,1,1] = sol3

        count += root_function(Z_trace, g, spl_neg, spl_pos, der_theta_F1, der_theta_F2, der_2_theta_F1, der_2_theta_F2, Lambda, theta_new, n, printy = False)[4]
...
def root_function(Z_trace, g, spl_neg, spl_pos, der_theta_F1, der_theta_F2, der_2_theta_F1, der_2_theta_F2, Lambda, theta, n, printy):
    N_particles = Z_trace.shape[1]
    d = theta.shape[1]
    count = 0
    f = np.zeros((d,d))
    ...
    for i in range(0,N_particles):
        ...
        if Z_trace[n,i,2] == g[n,i,0] and Z_trace[n,i,3] == g[n,i,1] and x_i[0] >= 0 :
            ...
            f_prime += (spl_pos(x_i[0], x_i[1], dx=1) * der_2_theta_F1(n, x_i[0], x_i[1], theta) +
                        spl_pos(x_i[0], x_i[1], dy=1) * der_2_theta_F2(n, x_i[0], x_i[1], theta)
                        )
        if Z_trace[n,i,2] == g[n,i,0] and Z_trace[n,i,3] == g[n,i,1] and x_i[0] < 0 :
            f += ( spl_neg(x_i[0], x_i[1], dx=1) * der_theta_F1(n, x_i[0], x_i[1], theta) +
                   spl_neg(x_i[0], x_i[1], dy=1) * der_theta_F2(n, x_i[0], x_i[1], theta)
                   )
            f_prime += (spl_neg(x_i[0], x_i[1], dx=1) * der_2_theta_F1(n, x_i[0], x_i[1], theta) +
                        spl_neg(x_i[0], x_i[1], dy=1) * der_2_theta_F2(n, x_i[0], x_i[1], theta)
                        )
...
def root_function_00(theta_00, Z_trace, g, spl_neg, spl_pos, der_theta_F1, der_theta_F2, der_2_theta_F1, der_2_theta_F2, Lambda, theta, n):
...
def root_function_01(theta_01, Z_trace, g, spl_neg, spl_pos, der_theta_F1, der_theta_F2, der_2_theta_F1, der_2_theta_F2, Lambda, theta, n):
...
def root_function_10(theta_10, Z_trace, g, spl_neg, spl_pos, der_theta_F1, der_theta_F2, der_2_theta_F1, der_2_theta_F2, Lambda, theta, n):
...
def root_function_11(theta_11, Z_trace, g, spl_neg, spl_pos, der_theta_F1, der_theta_F2, der_2_theta_F1, der_2_theta_F2, Lambda, theta, n):
...
def MFOC(N, d, T, dt, R, mu_0, center_left, center_right, y_left, y_right, xmin, xmax, grid_points, theta, F, mid_point, Lambda, num_iterations):
    ...
    for k in range(0,num_iterations):
        ...
        theta, f_value, count, loss_fct = parameter_update(Z_trace, g, psi_pos, psi_neg, der_theta_F1, der_theta_F2, der_2_theta_F1, der_2_theta_F2, theta, Lambda, x1 , x2, dx , dt, Nx, Nt, mid_point, dynamics_plots)
        counts[k] = count  
